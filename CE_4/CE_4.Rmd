...
The data has been collected at the wind farm Klim located close by Fjerritslev in Northwest of Jutland.
The measurements have been preprocessed so that the production and forecasts for the given hour is in the same row.
...
The initial hint was to estimate the power curve as function of wind direction and of the wind speed. Figure \ref{fig_1_1_1} shows a contour plot illustrating the power curve $p$ as function of the wind speed $Ws1$ and the wind direction $Wd1$ where `NA`s are excluded.
...
Figure \ref{fig_1_1} illustrates the power $p$ as function of wind speed $Ws1$. 
...
The optimization process is similar previous exercises where the estimate of the three parameters have been performed in order to minimize the SSE, eqn \ref{eq_1_2}. 
The optimizations are based upon following initial parameters: $L=`r round(initial_values[1],3)`$, $k=`r round(initial_values[2],3)`$ and $ws_0=`r round(initial_values[3],3)`$.
...
Figure \ref{fig_1_2} shows the four power curves as function of the wind speed for the given wind direction interval. 
...
tmp_top <- 10
```
I do not think the representation in figure \ref{fig_1_4} shows well representation of the power curve as function of the wind speed and of the wind direction. The highest recoded power production is $`r tmp[[1]][1]` [kW]$ with a wind speed of $`r tmp[[2]][1]` \left[ \frac{m}{s} \right]$ and with a wind direction of $`r tmp[[3]][1]` \left[ deg \right]$. 
...
A non-parametric approach has been carried out in order to create a better estimate of the multidimensional power curve. I have chosen to use the kernel smoothing with the Epanechnikov kernel, as we did in Computer Exercise 1.

There are several benefits by using a kernel estimate instead of the parametric estimate. E.g. the parametric approach has some limitation regarding the resolution of the wind direction. 
...
It has been chosen to randomly sample $`r sample_size*100`\%$ of complete cases of the total $`r nrow(X)`$ observations in order to increase computation time in the optimization process of $bw$. `r k_fold`-fold cross-validation have been used in order to validate the MSE1 of the randomly sampled observations. The observations have been divided into $`r (1-splt_test)*100`\%$ train data and $`r splt_test*100`\%$ validation data.
...
Due to the relative high range `r print(range(tmp_bw))` in the optimal values for each fold it has been decided to average value of the optimized $bw$. The average value is: $bw=`r opt_bw`$.
...
I think this can be solved by using a dynamic forgetting factor ($\lambda$). Although I start out with a static forgetting factor ($\lambda = `r lambda_opt`$) during the entire series.

It have been chosen to truncate estimates of the power curve from the kernel to a lower limit of $0$.
<!--
Figure \ref{fig_2_2} illutrates the power production as function of time.

```{r, fig.cap="\\label{fig_2_2}Plot power production as function of time."}
ggplot() +
...
\newpage
## Adaptive Recirsive Least Squares Regression

The goal is to make 1,2 and 3 hours ahead power production predictions. Due to the way the data is presented on the 1,2 and 3 hour predictions can be predicted as one step a head prediction. 
...
\begin{equation}
\begin{aligned}
R_t = \lambda_{\left( t \right)}R_{t-1} + X_tX_t^T
\end{aligned}
\label{eq_2_arls_2}
\end{equation}
\item The forgetting factor $\lambda$ determines the number of effective observations ($T_0$) in the estimation of $\theta_t$:
\begin{equation}
T_0 = \frac{1}{1-\lambda_{\left( t \right)}}
\label{eq_lambda}
\end{equation}
\end{itemize}
where $\lambda_{\left( t \right)}$ is a static or dynamic,  forgetting factor $\left( 0 <\lambda_{\left( t \right)} \le 1\right)$.

The following code chuck shows the implementation of the `ARLS`-function.

```{r, echo=TRUE}
ARLS <- function(x, Y=X$p, lambda=1, tau=NULL, PI=NULL) {
  # create matrices 
  ...
  for (t in 1:dim(x)[1]) {
    ...
    if (is.null(tau)) {
      ...
      Theta <- Theta + R^-1 %*% x[t, ] %*% rho(tau=tau, r=(Y[t] - t(x[t, ]) %*% Theta))
      }
    # trace theta
    theta_par[t, ] <- Theta
  }
...
where `x` is the input matrix. Quantile regression will be applied if `tau` is different from `NULL`. `lambda` have been chosen to keep constant in the first estimates process.
...
It has been chosen to focus on a subset of the predictions in order to better interpretable visualizations. The selected period is from: `r start_per` to `r end_per` and consist of `r per_n` samples.
...
```{r fig.cap="\\label{fig_2_3}Plot of the power and predicted power production as function of time."}
X %>%
...
```{r fig.cap="\\label{fig_2_5}Plot of the power and predicted power productions as function of time."}
X %>%
...
The 1, 2 and 3 hour predictions have more less same values in the selected period. All predicted series are positive shifted compared to the measured power production.
...
The model have been fitted for each of the three series as you see in figure \ref{fig_2_5}. 
To keep it simple, only the 1 hour prediction will be presented in the following analysis of the residuals.

Figure \ref{fig_2_8} illustrates the residuals as function of time.
```{r fig.cap="\\label{fig_2_8} Residuals as function of time.", message=FALSE, warning=FALSE}
ggplot(X) +
...
The plot in figure \ref{fig_2_8} shows a negative shift in the mean value ($`r round(mean(X$p_res_1),3)`$) of residuals. The negative shift is also supported in
figure \ref{fig_2_9} which shows a the distribution of the residuals.
...
A eloquent explanation for this would the constant forgetting factor $\lambda$ which is in the first model was set to $\lambda=1$. Then $\lambda=1$ the memory in the `ARLS` is $\infty$. See equation \ref{eq_lambda}.
...
The cumulated periodogram illustrate that the residuals are depending of each other, which involves unexplored systematic behaviour in the residuals. 
...
Due to the dynamic of the system it does not make sense to update the parameters with respect all the previous observations. By decreasing the value of $\lambda$ it is possible to discard "old" memory, see equation \ref{eq_lambda}.

```{r}
# Loss function
loss_func <- function(par, x_in, Y_in, tau_in=NULL) {
  res_tmp <- ARLS(x=x_in, Y=Y_in, lambda = par,tau = tau_in, PI = NULL)$epsilon
...
The MSE has decreased from $`r round(sse_tmp_1,4)`$ to $`r round(sse_tmp_2,4)`$ after the optimization of $\lambda$-values.

### Plot of the predictions

Figure \ref{fig_2_32} illustrates the prediction for the 1 hour series and the measured power production. The visual interpretation of the plot does not looks very different to the similar plot in figure \ref{fig_2_3}.

```{r fig.cap="\\label{fig_2_32}Plot of the power and predicted power production as function of time."}
X %>%
  select(t,p,p_pred_1,p_sd_1) %>%
  filter(t > start_per, t < end_per) %>%
  mutate(p_ymax = p_pred_1+p_sd_1,
...
```{r fig.cap="\\label{fig_2_52}Plot of the power and predicted power productions as function of time."}
X %>%
  select(t,p,p_pred_1,p_pred_2,p_pred_3) %>%
  filter(t > start_per, t < end_per) %>%
  ggplot() +
...
The visual similarity between figure \ref{fig_2_5} and figure \ref{fig_2_52} is high despite using the optimized forgetting values in each series. The predictions are still positive biased.
...
Figure \ref{fig_2_62} illustrates the estimated values of $\theta$ as a function of time. 
```{r fig.cap="\\label{fig_2_62} Estimated values of $\\theta$ as function of time."}
...
Figure \ref{fig_2_72} shows histogram for each of the estimated values of $\theta$.

```{r fig.cap="\\label{fig_2_72} Histograms of the estimated values of $\\theta$.", message=FALSE, warning=FALSE}
X %>%
  select(t,p,theta_1_alpha, theta_1_beta) %>%
  #mutate(theta_1_alpha=normalit(theta_1_alpha),
  #       theta_1_beta=normalit(theta_1_beta)) %>% 
...
Figure \ref{fig_2_82} illustrates the residual as function of time.
```{r fig.cap="\\label{fig_2_82} Residuals as function of time.", message=FALSE, warning=FALSE}
ggplot(X) +
  geom_line(aes(x=t, y=p_res_1, color="res_1H"), alpha=1/3) +
  labs(y="power [kW]", x="date time", color="Residauls") +
  theme_TS()
...
The mean value of the residuals is `r round(mean(X$p_res_1),3)` which is a slightly improvement to the model using $\lambda=1$. 
...
Figure \ref{fig_2_122} illustrates the cumulated periodogram of the residuals. There is still unexplored systematic behaviour left in the residuals.
...
\begin{itemize}
\item values of $\lambda$
\item huge jumps with missing values
\item 
\end{itemize}


\newpage
## Adaptive Quantile Recirsive Least Squares Regression

Quantile regression can be interpreted as a extension of linear regression. Quantile regression can be used when the conditions of linear regression are not appropriate. The motivation for quantile regression is the asymmetrical penalties which is needed for the non-linear periods in the power curve.
...
\begin{equation}
\rho_{ \tau  }\left( r \right) = \begin{cases} \tau r & r\ge 0 \\ \left(1- \tau \right) r & r<0 \end{cases}
\label{eq_qt_pen}
\end{equation}
...
It is now possible to estimate the parameters ($\theta$) for a given quantile $\tau$. 
It has been chosen to estimate $\theta$ for the following quantiles: $\tau = \left\{  0.05, 0.25, 0.50, 0.75, 0.95\right\}$. 

The `ARLS`-function have been applied in order to estimate the parameters for each of the values of $\tau$. $\lambda=1$ has been chosen for the initial model estimate by AQRLS.

```{r}
lambda_opt <- 1
# 1 hour ----
# 50
tmp <- ARLS(x=cbind(1, X$p_est_1), lambda=lambda_opt, PI=NULL, tau = 0.5)
# predictions
...
# 25
tmp <- ARLS(x=cbind(1, X$p_est_1), lambda=lambda_opt, PI=NULL, tau = 0.25)
# predictions
...
# 75
tmp <- ARLS(x=cbind(1, X$p_est_1), lambda=lambda_opt, PI=NULL, tau = 0.75)
# predictions
...
# 2 hour ----
# 50
tmp <- ARLS(x=cbind(1, X$p_est_2), lambda=lambda_opt, PI=NULL, tau = 0.5)
# predictions
...
# 25
tmp <- ARLS(x=cbind(1, X$p_est_2), lambda=lambda_opt, PI=NULL, tau = 0.25)
# predictions
...
# 75
tmp <- ARLS(x=cbind(1, X$p_est_2), lambda=lambda_opt, PI=NULL, tau = 0.75)
# predictions
...
# 3 hour ----
# 50
tmp <- ARLS(x=cbind(1, X$p_est_3), lambda=lambda_opt, PI=NULL, tau = 0.5)
# predictions
...
# 25
tmp <- ARLS(x=cbind(1, X$p_est_3), lambda=lambda_opt, PI=NULL, tau = 0.25)
# predictions
...
# 75
tmp <- ARLS(x=cbind(1, X$p_est_3), lambda=lambda_opt, PI=NULL, tau = 0.75)
# predictions
...
Figure \ref{fig_3_1} illustrates the same period of interest as in the ARLS regression. The QT bands for $\tau = 0.05$,  $\tau = 0.95$ and $\tau = 0.25$, $\tau = 0.75$ are plotted as shades in the figure. The QT where $\tau = 0.50$ has been plotted as a line to make it comparable with measured power production $p$. 

```{r fig.cap="\\label{fig_3_1}AQRLS estimates of $\\theta$ for given values of $\\tau$."}
X %>%
...
### Analysis of the residuals
Figure \ref{fig_3_2} illustrates the residuals as function of time for the 1 hour predictions.
```{r fig.cap="\\label{fig_3_2} Residuals as function of time.", message=FALSE, warning=FALSE}
X %>% 
...
Figure \ref{fig_3_3} illustrates the distribution of the residuals. As earlier there are a negative shift in the residuals.
...
Figure \ref{fig_3_4} shows the cumulated periodogram of the residuals. The qq-plot is quiet similar to figure \ref{fig_2_12} where $\lambda=1$. There are still systematic behaviour left in the residuals.
...
It has been chosen to count the number of predictions which are within, below or above the considered quantiles. Hereby it is possible to compare the performance of the AQRLS model with $\lambda=1$ to the AQRLS model where $\lambda$ has been optimized w.r.t. a minimization of the 1 step ahead prediction SSE. 
...
colnames(df_performance) <- c("Quantile (in %)","AQRLS (in %)")
...
# 1 hour ----
lambda_opt$p_est_1$qt05 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_1), Y_in=X$p, tau_in=0.05)$minimum
lambda_opt$p_est_1$qt25 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_1), Y_in=X$p, tau_in=0.25)$minimum
lambda_opt$p_est_1$qt50 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_1), Y_in=X$p, tau_in=0.5)$minimum
lambda_opt$p_est_1$qt75 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_1), Y_in=X$p, tau_in=0.75)$minimum
lambda_opt$p_est_1$qt95 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_1), Y_in=X$p, tau_in=0.95)$minimum
# 2 hour ----
lambda_opt$p_est_2$qt05 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_2), Y_in=X$p, tau_in=0.05)$minimum
lambda_opt$p_est_2$qt25 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_2), Y_in=X$p, tau_in=0.25)$minimum
lambda_opt$p_est_2$qt50 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_2), Y_in=X$p, tau_in=0.5)$minimum
lambda_opt$p_est_2$qt75 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_2), Y_in=X$p, tau_in=0.75)$minimum
lambda_opt$p_est_2$qt95 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_2), Y_in=X$p, tau_in=0.95)$minimum
# 3 hour ----
lambda_opt$p_est_3$qt05 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_3), Y_in=X$p, tau_in=0.05)$minimum
lambda_opt$p_est_3$qt25 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_3), Y_in=X$p, tau_in=0.25)$minimum
lambda_opt$p_est_3$qt50 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_3), Y_in=X$p, tau_in=0.5)$minimum
lambda_opt$p_est_3$qt75 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_3), Y_in=X$p, tau_in=0.75)$minimum
lambda_opt$p_est_3$qt95 <- optimize(loss_func, lower=0.6, upper=1, x_in=cbind(1, X$p_est_3), Y_in=X$p, tau_in=0.95)$minimum
```
```{r}
# table ----
df_lambda <- data.frame(matrix(NA, nrow = 3, ncol = 6))
df_lambda[1,] <- c("1H",
                   lambda_opt$p_est_1$qt05,
                   lambda_opt$p_est_1$qt25,
                   lambda_opt$p_est_1$qt50,
                   lambda_opt$p_est_1$qt75,
                   lambda_opt$p_est_1$qt95)
df_lambda[2,] <- c("2H",
                   lambda_opt$p_est_2$qt05,
                   lambda_opt$p_est_2$qt25,
                   lambda_opt$p_est_2$qt50,
                   lambda_opt$p_est_2$qt75,
                   lambda_opt$p_est_2$qt95)
df_lambda[3,] <- c("3H",
                   lambda_opt$p_est_3$qt05,
                   lambda_opt$p_est_3$qt25,
                   lambda_opt$p_est_3$qt50,
                   lambda_opt$p_est_3$qt75,
                   lambda_opt$p_est_3$qt95)
...
X %>%
  ...
  filter(t > start_per, t < end_per) %>%
  ggplot(aes(t)) + 
  geom_ribbon(aes(ymin=(p_pred_1_qt_005), ymax=(p_pred_1_qt_095),fill = "QT 5%-95%"), alpha=1) +
  geom_ribbon(aes(ymin=(p_pred_1_qt_025), ymax=(p_pred_1_qt_075),fill = "QT 25%-75%"), alpha=1) +
  scale_fill_manual("Bands",values=c("#999999","#E69F00")) +
  geom_line(aes(y=p, color="p"), alpha=1) +
  geom_line(aes(y=p_pred_1_qt_05, color="QT 50%"), alpha=1) +
  #geom_line(aes(y=p_pred_1, color="pred_1"), alpha=1) +
...
Parameter values as function of time.. Only for the wd1 ws1...

Figure \ref{fig_3_3} illustrates the normlized values of $\theta$.
```{r fig.cap="\\label{fig_3_3} Normalized values of $\theta$ as function of time."}
X %>%
  select(t,p,theta_1_alpha, theta_1_beta) %>%
  mutate(theta_1_alpha=normalit(theta_1_alpha),
         theta_1_beta=normalit(theta_1_beta)) %>% 
  #filter(t > start_per, t < end_per) %>%
  ggplot() +
  geom_line(aes(x=t, y=theta_1_alpha, color="alpha"), alpha=1/3) +
  geom_line(aes(x=t, y=theta_1_beta, color="beta"), alpha=1/3) +
  labs(y="Value", x="date time", color="Normalized thetas") +
  theme_TS()
```

Figure \ref{fig_3_4} illustrates histograms of the normlized values of $\theta$.
```{r fig.cap="\\label{fig_3_4} Histograms of the normlized values of $\theta$.", message=FALSE, warning=FALSE}
X %>%
  select(t,p,theta_1_alpha, theta_1_beta) %>%
  mutate(theta_1_alpha=normalit(theta_1_alpha),
         theta_1_beta=normalit(theta_1_beta)) %>% 
  filter(t > start_per, t < end_per) %>%
  ggplot() +
  geom_histogram(aes(x=theta_1_alpha, colour="alpha"), alpha=1/4) +
  geom_histogram(aes(x=theta_1_beta, colour="beta"), alpha=1/4) +
  #scale_y_continuous(labels = scales::percent) +
  labs(y="frequency", x="theta value", color="theta parameters") +
  theme_TS()
```
<!--
