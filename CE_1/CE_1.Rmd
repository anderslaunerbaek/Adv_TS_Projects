...
## Part 1
There is generated $n=1000$ noise samples from a $x \sim \mathcal{N}(0,\,1)$ which is used in all simulation in part one. 
...
The model (eq. \ref{eq_1_SETAR_r}) have been simulated with two different set of parameters (eq. \ref{eq_1_2a} - eq. \ref{eq_12_2b}) and their processes are plotted in fig. \ref{fig_ex_2}. 
...
\item It is also possible to see the inverse properties of the two model defined by the slope. 
...
\begin{itemize}
\item Different from the SETAR model the IGAR model using a given external parameter to switch between regimes. In this case there have been sampled a random number between $[0,1]$. 
...
\begin{itemize}
\item The main difference between the three models are the properties for shifting to a new regime. The shift in the SETAR model depends on the previous value of the model and some predefined regimes and the IGAR model and the MMAR model shifts with respect to an external parameter.
\item The main difference between the IGAR model and the MMAR model is that it is possible to determine different thresholds for shifting between given regimes. Maybe there are some physical properties which make the transition from a given stage to another given stage.
\end{itemize}
...
The theoretical mean, eq. \ref{eq_2_theo_mu}

\begin{equation}
M(x) = E\left\{  X_{t + 1} |X_t = x\right\} 
\label{eq_2_theo_mu}
\end{equation}
By the fact that the noise are Gaussian distributed, $\epsilon _{ t } =0$ it is possible to rewrite the SETAR(2,1,1) model (eq. \ref{eq_1_SETAR_r}) to the theoretical mean in eq. \ref{eq_2_theo_mu_r}
...
ggplot(data) +
  geom_point(aes(x = y_SETAR_1, y = y_SETAR, color = "SETAR(2,1,1), par_1"), alpha = 1/2) +
  geom_line(aes(x = y_SETAR_1, y = mean_theo, color = "M_t"), alpha = 1/2) +
  labs(x = "y(t - 1)", y = "y(t)", color = "") +
  theme_TS()

```

Fig. \ref{fig_ex_5} shows the plot of the theoretical mean with two. I do not have any comments to this plot.
...
The `lm()` uses the bandwidth restricts number of samples to include to each fit $idx =as.integer(bw_n*n)$. Besides the bandwidth each fit also uses weigths from the Epanechnikov kernel (function from sample code).
...
Fig. \ref{fig_ex_6} shows the plot of the estimated mean with two different bandwidths.
...
\item A higher bandwidth will decrease the noise in the signal and vice versa. The high bandwidth makes estimated mean is less flexibel to regime shifts for the SETAR(2,1,1) model.
\item A lower bandwidth is more suitable to estimate the mean value of the SETAR(2,1,1) model, due to the charaterica of its regimes.
\item It is also posssible to obtain better estimation in the boundaries by using the lower bandwidth.
\end{itemize}

\newpage
## Part 3
The `cumulativeMeans.R` script have been used to calculate the cumulative conditional means.

The theoretical cumulative conditional means are determined by eq. \ref{eq_3_1}.
\begin{equation}
sd
\label{eq_3_1}
\end{equation}

```{r, fig.cap="\\label{fig_ex_7}Plots of the theoretical cumulative conditional means and estimated cumulative conditional means."}
...
while(!all(sapply(L, length) >= 5)) {
  ...
  ## Cut into intervals conditioned on x_{t-1}
  L <- split(data$y_SETAR, cut(data$y_SETAR_1, breaks))
  L_theo <- split(data$mean_theo, cut(data$mean_theo_1, breaks))
}
...
ggplot(data) +
  ...
  labs(x = "y(t - 1)", y = "y(t)", color = "") +
  theme_TS() +
  geom_line(data = confi, aes(x = breaks, y = cum_con_mean, color = "M_t_cum"), alpha = 1) +
  geom_line(data = confi, aes(x = breaks, y = cum_con_mean_theo, color = "M_t_cum_theo"), alpha = 1)
```


Fig. \ref{fig_ex_7} shows the plot of the theoretical cumulative conditional means and the estimated cumulative conditional means.
\begin{itemize}
\item 
\end{itemize}
...
where $X_t$ is the input and $Y_t$ is the output. I assume the input are more less constant and given by $\mathcal{U}(0.95,\,0.99)$. $g(x)$ is defined in eq. \ref{eq_4_2}.
\begin{equation}
g(x) = 4x(1 - x)
\label{eq_4_2}
\end{equation}
...
yprd <- outer(x1Seq, y1Seq, function(x1,y1) {
  L <- lapply(1:length(x1), function(i) {
    ...
    ok <- w > 0
    ## Note that this is local first order polynomial regression, but can easily be made 2'nd
    fit <- lm(model_4 ~ model_4_in_1 + model_4_1, weights=w[ok], data = subset(data, subset = 1:n %in% idx)[ok, ])
...
mtrx.melt <- reshape2::melt(yprd_non, id.vars = c("X_t1", "Y_t1"), measure.vars = "Y_t")
names(mtrx.melt) <- c("X_t1", "Y_t1", "Y_t")
mtrx.melt$X_t1 <- x1Seq[mtrx.melt$X_t1]
mtrx.melt$Y_t1 <- y1Seq[mtrx.melt$Y_t1]
...
I have tried to apply the `ACF()` on the SETAR(2,1,1) model from part one and it was clearly measure some deregee of linear dependency. I have therefore chosen the model from part 4 ($Y_t$ from eq. \ref{eq_4_1}).

```{r, fig.cap="\\label{fig_ex_11}ACF of the SETAR(2,1,1) model from part one."}
# decrease the data set agian..
if (nrow(data) == 2999) {
  n <- nrow(data)
  data <- subset(data, subset = 1:n %in% 1:1000)
  n <- nrow(data)
}

#
lags_acf <- data.frame(lag = acf(data$model_4, plot = F)$lag,
                       acf = acf(data$model_4, plot = F)$acf,
                       confi = 2/sqrt(n), # from introduction to times series
                       zero = 0)
...
```{r, message=FALSE, fig.cap="\\label{fig_ex_12}"}
...
Fig. \re{fig_ex_12} shows the LDF of the model $Y_t$ from eq. \ref{eq_4_1}.
\begin{itemize}
\item 
\end{itemize}
