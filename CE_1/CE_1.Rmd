...
```
## Part 1
There is simulated $n=1000$ $\epsilon_t \sim \mathcal{N}(0,\,1)$ which is used as noise input for all simulations in part one.
...
### IGAR(2,1)
The IGAR model are given by the same equation as the SETAR model (eq. \ref{eq_1_SETAR}) but using an external parameter to switch between regimes. The external shift parameter is in this case a random variable $p \sim \mathcal{U}(0,\,1)$. 
...
There has been simulated a process of eq. \ref{eq_4_1} and I would like to find the dependence of $Y_t$ on $X_{t-1}$ and $Y_{t-1}$. The depences will be discovered using local regression and a contour plot. 

I have chosen to use the `lm()` with the Epanechnikov kernel with the bandwidth is chosen to `r bw`. 
I assume there is a local linear relation between $X_{t-1}$ and $Y_{t-1}$ therefore the first order regression: `lm(Y_t ~ X_{t-1} + Y_{t-1}, weights=w[ok], data = data[ok, ])` and as in the example there will only be consdiered samples which have weigths greather than zero.
...
Instead of the 3D visualisation I have chosen to visulise $Y_t$ dependence on $X_{t-1}$ and $Y_{t-1}$ in a contour plot. Fig. \ref{fig_ex_8}
\begin{itemize}
\item The contour lines are more less constant in 
\end{itemize}

### Conditional parametric model approach

I have used the following function call in the conditional parametric approach: `loess(Y_t ~ X_{t-1} + Y_{t-1}, span = bw, degree = 1, data = data)`. 

```{r, fig.cap="\\label{fig_ex_8_1}"}
## Do local 1'st order regression. This can also easily be changed to a conditional parametric model
fit <- loess(model_4 ~ model_4_in_1 + model_4_1, span = bw, degree = 1,
             data = data[, c("model_4", "model_4_in_1", "model_4_1")], parametric = "model_4", family = "gaussian")
