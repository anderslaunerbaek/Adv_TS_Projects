...
Når der kommer støj bliver den skubbert væk fra 

Minimere det fysiske potentiale


## Part 1: Simulation and discretization of di usion processes
```{r}
...
model_func <- function(sigma, delta, t, Theta, init_values) {
  ...
  re_plot <- ggplot2::ggplot(data) + 
    geom_point(aes(x = T, y = Y_1, color = "Y_k^1"), alpha = 1/2) +
    geom_point(aes(x = T, y = Y_2, color = "Y_k^2"), alpha = 1/2) +
    labs(x = "t", y = "Y_k^*(t) ", color = "") +
    theme_TS()
  
  # phase
  ph_plot <- ggplot2::ggplot(data) + 
    geom_point(aes(x = Y_1, y = Y_2, color = "Phase"), alpha = 1/2) +
    labs(x = "Y_k^1(t)", y = "Y_k^2(t)", color = "") +
    theme_TS()
  return(list("sim" = data,
              "re_plot" = re_plot,
              "ph_plot" = ph_plot))
}
```
