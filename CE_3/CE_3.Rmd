...
```
The relizations of $Y_k^1$ and $Y_k^2$ wrt. time and phase plots of $Y_k^1$ and $Y_k^2$ are constructed for different $\sigma=[0.0,\,0.1,\,0.2,\,0.3,\,0.4]$.
...
tmp$ph_plot
```
$\sigma = 0.0$ returns a stable cyclic system. It takes $\approx10$ time steps for the system to converge to its cyclic pattern. The $\approx10$ time steps will be the same for all simulations. This is caused by the intial parameters of $Y_0^1$ and $Y_0^2$.
...
Changing $\sigma$ to $0.20$ does make an effect in both the realizations and in the phase plot. The time sereis still needs $\approx10$ time steps to converge to its normal cyclic pattern. The pattern changes dramatically after that for some cycles, back to normal and then back to the abnormal pattern agian. The cahnge in $\sigma$ from $0.10$ to $0.20$ increase the effect of the wiener process.
...
tmp$ph_plot
```
Changing $\sigma$ from $0.20$ to $0.30$ gives more less the same illustrations as before. It is noticable to see a greater variance $Y_k^2$ when $Y_k^1$ is in the range $[-1.75;\,1.5]$.
...
The length of the abnormal cycles has great influce of the amplitude of the time series. The amplitudes are more similar to the normal pattern when the abnormal pattern is short. For longer peroid of abnormal pattern entail smaller amplitudes. The smaller amplitudes creates more cycles in the lower rigth corner of the phase plot.

\newpage
#### Comment on the effect of adding noise to the equations
\begin{itemize}
\item The first and most noticeable visual change by increasing the noise ($\sigma$) is the amplitude and the nnumber of cycles in the abnormal pattern. Those effecting the "number of cycles" in the abnormal pattern where greater values of $\sigma$ ential more frequent change between the normal pattern and the abnormal pattern.
...
\end{itemize}
The sample perid is $10 \left[min\right]$.
...
Figure \ref{fig_ex_2_1} shows the interesting recorded time series in three subplots:
\begin{itemize}
\item A step seqence which tells when the heater is on $Ph$ in the east room.
...
It is possible to see how the solor radiation, on surface facing south, increases the temperatures and the pattern for the heater in figure \ref{fig_ex_2_1}. 

\newpage
#### 2. Step

There has been implmeneted the most simple model of the system in step 2. 

Equantion \ref{eq_2_2a_1} and eqn. \ref{eq_2_2a_2} shows the system eqn. and measurement eqn. respectively.

\begin{equation}
dT_i=\left( \frac{1}{R_{ia}C_i}\left(T_a-T-i \right) +\frac{1}{C_i}\Phi_h  \right)dt+\sigma_id\omega_i
\label{eq_2_2a_1}
\end{equation}
\begin{equation}
...
I fixed this issue by increasing the limits on both of the following two parameters $Ria$ and $Ti0$. Then estimate the model agian.
...
The estimated values of the two affected parameters are now $Ti0 = 24.502$, which was close to a initial binding value for $Ti0$, and $Ria = 26.260$ which is roughly about twice as munch as the inital upper limit value for $Ria$. 

By fixing the initiallimit issues also increased the loglikelihood significantly and decreased the correlations among the parameters which is desirable.

\newpage
#### 4. Step
The estimated model is analyzed in step 4. We are interested in the residuals of the one step ahead prediction.
```{r, echo=TRUE}
...
Figure \ref{fig_ex_2b_1} shows a time serie plot of residuals from the one step ahead predictions. It is clear to see that the residuals if not white noise which indicates that there is some systematic behavoir left.
...
```{r, fig.cap="\\label{fig_ex_2b_2}Histogram of the residuals."}
...
Figure \ref{fig_ex_2b_6} shows the cumulated periodogram of the residuals. It is possible to see that the residuals are dependent of each other, which identicate that there is still unexpolred systematic behavoir left in the residuals. The residuals will be on the diagonal in cumulated periodogram if they are white noise.
